...
######################################################################
declare -i count #index number per child per day
for input in "$@"
do
    count=1 #[Re]Set index number per child
    while IFS='' read -r line || [[ -n "$line" ]]; do
        ...
        JPEG)
            echo "TYPE is a jpg"
            echo "[CMD] mv $$.tmp Kiddie_Academy_${input}_${count}.jpg"
            mv $$.tmp Kiddie_Academy_${input}_${count}.jpg
            ;;
        ISO)
            echo "TYPE is a mp4"
            echo "[CMD] mv $$.tmp Kiddie_Academy_${input}_${count}.mp4"
            mv $$.tmp Kiddie_Academy_${input}_${count}.mp4
            ;;
        *)
